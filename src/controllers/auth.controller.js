...
export async function signup(req, res) {
    ...
    if (existingUser) {
        return res.status(400).json({ errors: [{ message: 'User with this email already exist.' }] })
    }
    ...
    try {
        ...
        if (!newUser) {
            return res.status(400).json({ errors: [{ message: 'Invalid user data.' }] });
        ...
        return res.status(201).json({ data: { user: userDTO } });
    } catch (error) {
...
export async function login(req, res) {
    ...
    try {
        ...
        return res.status(200).json({ data: { user: userDTO } });
    } catch (error) {
